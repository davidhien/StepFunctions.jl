...
@testset "StepFunctions.jl" begin
    ...
    @testset "restrict" begin
        f = StepFunction(Float64[], [1])

        g = restrict(f, -Inf, Inf)
        @test g == f

        g = restrict(f, -Inf, 0)
        @test g == StepFunction([0], [1, 0])

        g = restrict(f, 0, Inf)
        @test g == StepFunction([0], [0, 1])


        f = StepFunction([1, 2], [1, 2, 3])

        # exactly on breakpoints
        g = restrict(f, 1, Inf)
        @test g == StepFunction([1, 2], [0, 2, 3])
        g = restrict(f, -Inf, 2)
        @test g == StepFunction([1, 2], [1, 2, 0])

        # in between breakpoints
        g = restrict(f, 1.5, Inf)
        @test g == StepFunction([1.5, 2], [0, 2, 3])
        g = restrict(f, -Inf, 1.5)
